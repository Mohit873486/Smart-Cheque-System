...
public class ChequeOcrService {
    ...
    public ChequeOcrService() {
        ...
        // datapath: use TESSDATA_PREFIX env or default
    ...
    public ChequeOcrResult extractFromFile(File file) throws OcrException {
        try {
            ...
            if (img == null)
                throw new OcrException("Unable to read image file.");
    ...
    private BufferedImage preprocessImage(BufferedImage in) {
        ...
        ColorConvertOp op = new ColorConvertOp(in.getColorModel().getColorSpace(), gray.getColorModel().getColorSpace(),
                null);
        op.filter(in, gray);
    ...
    private Optional<String> extractChequeNumber(String text) {
        ...
        Pattern p = Pattern.compile("(CHQ[-\\s]?\\d{2,}|Cheque\\s*(No|#|Number)[:\\s]*([A-Za-z0-9-]+))",
    ...
    private Optional<BigDecimal> extractAmount(String text) {
        ...
        Pattern p = Pattern.compile("[₹$]?\\s*([0-9]{1,3}(?:[,\\.]?[0-9]{3})*(?:[.,][0-9]{1,2})?)",
                Pattern.CASE_INSENSITIVE);
        Matcher m = p.matcher(text);
    ...
    private Optional<LocalDate> extractDate(String text) {
        ...
        if (m.find()) {
            ...
            for (var fmt : fmts) {
                try {
                    LocalDate dt = LocalDate.parse(candidate,
                            fmt.withResolverStyle(java.time.format.ResolverStyle.SMART));
