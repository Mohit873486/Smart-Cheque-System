...
public class CommandDetectionService {
    ...
    /**
     * Analyzes the user input and determines if it matches a predefined local
     * command.
     * 
    ...
    public CommandType detectCommand(String input) {
        ...
        if (normalized.contains("create cheque") || normalized.contains("add cheque")
                || normalized.contains("new cheque")) {
     ...
     * In a full implementation, this class would take dependencies (like DAO or
     * other services)
    ...
    public String executeCommand(CommandType command) {
        ...
        switch (command) {
            ...
            case SHOW_PENDING_CHEQUES:
                // TODO: Fetch results from Database using ChequeResponse
